...
contract StableMath {
    ...
    // CERTORA HELPERS
    function getIndex(uint256[] memory arr, uint256 i) public pure  returns(uint256) {
        return arr[i];
    }

    function getLength(uint256[] memory arr) public pure returns(uint256) {
        return arr.length;
    }

    function getFloor(uint256[] memory arr) external pure returns (uint256 min) {
        min = uint256(-1);
        ...
        min=Math.mul(min,arr.length);
    }

    function getSum(uint256 numTokens, uint256[] memory balances) public pure returns(uint256 sum) {
        ...
        returns (uint256)
    {
        ...
        **********************************************************************************************/
        ...
        (invariant, prevInvariant) = loopIt(invariant, numTokens, ampTimesTotal, sum, balances);
    ...
    function loopIt(
    ...
    ) public pure returns(uint256,uint256 prevInvariant) {
        uint256 D_P = invariant;
        ...
        invariant = Math.divDown(
            ...
            Math.divDown(
                Math.mul((ampTimesTotal - _AMP_PRECISION), invariant), _AMP_PRECISION
